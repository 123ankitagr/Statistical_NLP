...
class sample(object):
    def __init__(self, inst):
        ...
        p = inst.position
        context = [tuple[0] for tuple in inst.context[p - 5:p] if
                   len(tuple) > 1]  # checking if list element is actually a tuple, phrasal elements are not
...
class EM(object):
    ...
    def E_step(self, samples):
        ...
        for sample in samples:
            ...
            context_given_sense = np.prod(words_given_sense, axis=0)  # P(CiSk)
            # multiply by priors
            context_probs = np.multiply(context_given_sense, priors)  # P(CiSk) * P(Sk)
            marginal = sum(context_probs)
            for k in range(0, len(context_probs)):
                H[i][k] = context_probs[k] / marginal
                # print(H[i][k])
            i += 1
